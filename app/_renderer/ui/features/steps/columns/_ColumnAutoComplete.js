...
import styles from './_ColumnAutoComplete.scss';
...
class ColumnAutoComplete extends React.Component {
  constructor(props) {
    super(props);
  ...
  handleSelection(idx) {
    ...
    const part2 = this.props.suffix;
    const part3 = value.substring(matchStart + matchLength);

    const event = { target: this.refInput };

    event.target.value = `${part1}${slug}${part2} ${part3}`;
    this.handleChange(event);
    this.resetHelper();
    this.updateCaretPosition(part1.length + slug.length + part2.length);
    this.enableSpaceRemovers = true;
  }

  updateCaretPosition(caret) {
    this.setState({ caret }, () => setCaretPosition(this.refInput, caret));
  }
  ...
  renderAutocompleteList() {
    ...
    const helperOptions = options.slice(0, optionNumber).map((val, idx) => {
      ...
      return (
        <li
          className={idx === selection ? styles.activeInput : null}
          key={val}
    ...
    return (
      <ul className={styles['react-autocomplete-input']} style={{ left: left + offsetX, top: top + offsetY }}>
        {helperOptions}
      </ul>
    );
  }
  ...
  render() {
    const { defaultValue, disabled, value } = this.props;
    ...
    return (
      <span>
        <input
          className="pt-input"
          type="text"
          placeholder="Template input $[fieldName]"
          disabled={disabled}
...
ColumnAutoComplete.propTypes = propTypes;
ColumnAutoComplete.defaultProps = defaultProps;

export default ColumnAutoComplete;
